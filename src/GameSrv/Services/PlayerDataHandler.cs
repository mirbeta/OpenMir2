namespace GameSrv.Services
{
    ...
    public static class PlayerDataHandler
    {
        ...
        public static void Enqueue(int queryId, ServerRequestData data)
        {
            ...
            LogService.Debug($"查询角色数据:{data.QueryId}成功");
        }
        ...
        {
            //todo 保存数据优化一下流程，M2Server.需等待DBSrv结果，然后异步通知
        ...
        public static void ProcessQueryData()
        {
            if (QueryProcessList.TryDequeue(out QueryPlayData queryData))
            {
                if (queryData.QueryCount >= 60) //60次后放弃
                {
                    LogService.Warn($"查询角色数据[{queryData.ChrName}/QueryId:{queryData.QueryId}] 超时...");
                    return;
                }
                int nIdent = 0;
                int nRecog = 0;
        ...
        private static bool LoadRcd(LoadCharacterData loadHuman, ref int queryId)
        {
            ...
            if (GameShare.DataServer.SendRequest(queryId, packet, loadHuman))
            {
                ...
                {
                    QueryId = queryId,
                    ChrName = loadHuman.ChrName
                });
                LogService.Debug($"查询角色[{loadHuman.ChrName}]数据 任务Id:[{queryId}]");
                return true;
            }
            LogService.Warn("DBSvr链接丢失，请确认DBSvr服务状态是否正常。");
            return false;
