...
namespace SystemModule.CoreSocket
{
    ...
    public abstract class ClientFactory<TClient> : DisposableObject where TClient : IClient
    {
        ...
        {
            return Task.Run(() => { return this.CheckStatus(tryInit); });
        }
        ...
        {
            return Task.Run(() => { return this.Clear(); });
        }
        ...
        {
            int count = 0;
            foreach (TClient item in this.CreatedClients)
            {
                count++;
                DisposeClient(item);
            }
        ...
        {
            return Task.Run(() => { this.DisposeClient(client); });
        }
        ...
        {
            return Task.Run(() => { this.ReleaseTransferClient(client); });
        }
