...
namespace GameSvr.Player
{
    public partial class PlayObject
    {
        ...
        public PlayObject()
        {
            ...
            OnHorse = false;
            MWContribution = 0;
            RankLevelName = M2Share.g_sRankLevelName;
            FixedHideMode = true;
            MNVal = new int[100];
            MNMval = new int[100];
            MDyVal = new int[100];
        ...
        private byte[] smallData = new byte[32];
        ...
        private void SendSocket(string sMsg)
        {
            try
            {
                ...
                var actionPacket = new ActionOutPacket();
                actionPacket.MessagePacket = messageHead;
                var msgBuff = HUtil32.GetBytes(sMsg);
                MemoryCopy.BlockCopy(ServerPackSerializer.Serialize(messageHead), 0, sendData, 0, 20);
                MemoryCopy.BlockCopy(msgBuff, 0, sendData, 20, msgBuff.Length);
                M2Share.GateMgr.AddGateBuffer(GateIdx, sendData[..(msgBuff.Length + 20)]);
            }
            catch (Exception ex)
            {
        ...
        private void SendSocket(ClientCommandPacket defMsg)
        {
            ...
            MemoryCopy.BlockCopy(ServerPackSerializer.Serialize(messageHead), 0, smallData, 0, ServerMessagePacket.PacketSize);
            MemoryCopy.BlockCopy(ServerPackSerializer.Serialize(defMsg), 0, smallData, ServerMessagePacket.PacketSize, ClientCommandPacket.PackSize);
            M2Share.GateMgr.AddGateBuffer(GateIdx, smallData);
        }
        ...
        internal virtual void SendSocket(ClientCommandPacket defMsg, string sMsg)
        {
            ...
            if ((bMsg.Length + HeaderLen) > MaxSendLen)
            {
                var packetLen = bMsg.Length;
                var sendLen = 0;
                M2Share.GateMgr.AddGateBuffer(GateIdx, sendData[..HeaderLen]);//先发送消息头
                while (packetLen > 0)
                {
                    var copyLen = packetLen < MaxSendLen ? packetLen : MaxSendLen;
                    if (copyLen < MaxSendLen)
                    {
                        MemoryCopy.BlockCopy(bMsg, sendLen, sendData, 0, copyLen);
                        M2Share.GateMgr.AddGateBuffer(GateIdx, sendData[..copyLen]);
                    }
                    else
                    {
                        MemoryCopy.BlockCopy(bMsg, sendLen, sendData, 0, copyLen);
            ...
            else
            {
                ...
                M2Share.GateMgr.AddGateBuffer(GateIdx, sendData[..(HeaderLen + bMsg.Length)]);//小包直接发送
            }
        }
