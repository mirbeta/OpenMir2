...
public class FileCachePool : DisposableObject
{
    ...
    public bool Find(string key, out byte[] data)
    {
        using (new ReadLock(lockEx))
        {
            ...
            if (!entriesByKey.TryGetValue(key, out MemCacheEntry cacheValue))
            {
                data = null;
                return false;
            }
    ...
    public void Clear()
    {
        using (new WriteLock(lockEx))
        {
            ...
            foreach (KeyValuePair<string, FileCacheEntry> fileCacheEntry in pathsByKey)
    ...
    private class FileCacheEntry
    {
        ...
        {
            string key = e.FullPath.Replace(entry._path, entry._prefix);
        ...
        {
            string oldKey = e.OldFullPath.Replace(entry._path, entry._prefix);
            string oldFile = e.OldFullPath;
            string newKey = e.FullPath.Replace(entry._path, entry._prefix);
    ...
    private bool RemovePathInternal(string path)
    {
        using (new WriteLock(lockEx))
        {
            ...
            if (!pathsByKey.TryGetValue(path, out FileCacheEntry cacheValue))
                return false;
            ...
            // Remove path entries
            foreach (string entryKey in entriesByPath[path])
