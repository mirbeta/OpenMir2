...
namespace SystemModule.CoreSocket
{
    ...
    public class TcpService<TClient> : TcpServiceBase, ITcpService<TClient> where TClient : SocketClient
    {
        ...
        private SystemModule.CoreSocket.Common.NetworkMonitor[] m_monitors;
        ...
        /// <inheritdoc/>
        /// </summary>
        public override SystemModule.CoreSocket.Common.NetworkMonitor[] Monitors => m_monitors;
        ...
        public override IService Stop()
        {
            if (m_monitors != null)
            {
                foreach (SystemModule.CoreSocket.Common.NetworkMonitor item in m_monitors)
                {
                    item.Socket?.Dispose();
                }
            }
        ...
        private void BeginListen(IPHost[] iPHosts)
        {
            ...
            List<SystemModule.CoreSocket.Common.NetworkMonitor> networkMonitors = new List<SystemModule.CoreSocket.Common.NetworkMonitor>();
            foreach (IPHost iPHost in iPHosts)
            {
                ...
                networkMonitors.Add(new SystemModule.CoreSocket.Common.NetworkMonitor(iPHost, socket));
            }

            m_monitors = networkMonitors.ToArray();
            foreach (SystemModule.CoreSocket.Common.NetworkMonitor networkMonitor in m_monitors)
            {
