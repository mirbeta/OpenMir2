...
namespace M2Server.Player
{
    public partial class PlayObject
    {
            ...
            if (wHitMode > 0)
            {
                switch (wHitMode)
                {
                    case 4:// 刺杀
                        ...
                        if (nSecPwr > 0)
                        {
                            if (!SwordLongAttack(ref nSecPwr) && ModuleShare.Config.LimitSwordLong)
                            {
                                wHitMode = 0;
                            }
                        }
        ...
        private void AttackSuccess(short wHitMode, int nPower, bool canHit, IActor targetObject)
        {
            ...
            if (!targetObject.UnParalysis && Paralysis && (M2Share.RandomNumber.Random(targetObject.AntiPoison + ModuleShare.Config.AttackPosionRate) == 0))
            {
                targetObject.MakePosion(PoisonState.STONE, ModuleShare.Config.AttackPosionTime, 0);
            }
            ...
            if (ModuleShare.Config.MonDelHptoExp)
            {
                switch (Race)
                {
                    case ActorRace.Play:
                        if (IsRobot)
                        {
                            if (((RobotPlayer)this).Abil.Level <= ModuleShare.Config.MonHptoExpLevel)
                            {
                                if (!M2Share.GetNoHptoexpMonList(targetObject.ChrName))
                                {
                                    ((RobotPlayer)this).GainExp(nPower * ModuleShare.Config.MonHptoExpmax);
                                }
                            }
                        }
                        else
                        {
                            if (Abil.Level <= ModuleShare.Config.MonHptoExpLevel)
                            {
                                if (!M2Share.GetNoHptoexpMonList(targetObject.ChrName))
                                {
                                    GainExp(nPower * ModuleShare.Config.MonHptoExpmax);
                                }
                            }
                        }
                        break;
                    case ActorRace.PlayClone:
                        if (Master != null)
                        {
                            if (Master.IsRobot)
                            {
                                if (((RobotPlayer)Master).Abil.Level <= ModuleShare.Config.MonHptoExpLevel)
                                {
                                    if (!M2Share.GetNoHptoexpMonList(targetObject.ChrName))
                                    {
                                        ((RobotPlayer)Master).GainExp(nPower * ModuleShare.Config.MonHptoExpmax);
                                    }
                                }
                            }
                            else
                            {
                                if (((PlayObject)Master).Abil.Level <= ModuleShare.Config.MonHptoExpLevel)
                                {
                                    if (!M2Share.GetNoHptoexpMonList(targetObject.ChrName))
                                    {
                                        ((PlayObject)Master).GainExp(nPower * ModuleShare.Config.MonHptoExpmax);
                                    }
                                }
                            }
                        }
        ...
        protected override bool IsAttackTarget(IActor targetObject)
        {
            ...
            switch (AttatckMode)
            {
                case AttackMode.HAM_ALL:
                    ...
                    if (ModuleShare.Config.PveServer)
                    {
                        return true;
                    }
                    break;
        ...
        public override bool IsProperFriend(IActor targetObject)
        {
            if (targetObject.Race == ActorRace.Play)
            {
                ...
                {
                    if (ModuleShare.Config.boPKLevelProtect)// 新人保护
                    {
                        if (Abil.Level > ModuleShare.Config.nPKProtectLevel)// 如果大于指定等级
                        {
                            if (!playObject.PvpFlag && targetObject.WAbil.Level <= ModuleShare.Config.nPKProtectLevel && playObject.PvpLevel() < 2)// 被攻击的人物小指定等级没有红名，则不可以攻击。
                            {
                                return false;
                            }
                        }
                        if (Abil.Level <= ModuleShare.Config.nPKProtectLevel)// 如果小于指定等级
                        {
                            if (!playObject.PvpFlag && targetObject.WAbil.Level > ModuleShare.Config.nPKProtectLevel && playObject.PvpLevel() < 2)
                            {
                                return false;
                            }
                        }
                    ...
                    if (PvpLevel() >= 2 && Abil.Level > ModuleShare.Config.nRedPKProtectLevel)
                    {
                        if (targetObject.Abil.Level <= ModuleShare.Config.nRedPKProtectLevel && playObject.PvpLevel() < 2)
                        {
                            return false;
                        }
                    }
                    // 小于指定级别的非红名人物不可以杀指定级别红名人物。
                    if (Abil.Level <= ModuleShare.Config.nRedPKProtectLevel && PvpLevel() < 2)
                    {
                        if (playObject.PvpLevel() >= 2 && targetObject.Abil.Level > ModuleShare.Config.nRedPKProtectLevel)
                        {
                            return false;
                        }
                    }
            ...
            try
            {
                ...
                if (!ModuleShare.Config.CloseSpeedHackCheck)
                {
                    if (!boLateDelivery)
                    {
                        ...
                        IsFilterAction = true;
                        int dwAttackTime = HUtil32._MAX(0, ModuleShare.Config.HitIntervalTime - HitSpeed * ModuleShare.Config.ItemSpeed);
                        ...
                        if (dwCheckTime < dwAttackTime)
                        {
                            ...
                            if (delayTime > ModuleShare.Config.DropOverSpeed)
                            {
                                if (AttackCount >= 4)
                                {
                                    ...
                                    AttackCount = 0;
                                    delayTime = ModuleShare.Config.DropOverSpeed;
                                    if (TestSpeedMode)
                                    {
        ...
        private bool ClientHorseRunXY(int wIdent, short nX, short nY, bool boLateDelivery, ref int delayTime)
        {
            ...
            if (!ModuleShare.Config.CloseSpeedHackCheck)
            {
                if (!boLateDelivery)
                {
                    ...
                    if (dwCheckTime < ModuleShare.Config.RunIntervalTime)
                    {
                        MoveCount++;
                        delayTime = ModuleShare.Config.RunIntervalTime - dwCheckTime;
                        if (delayTime > ModuleShare.Config.DropOverSpeed)
                        {
                            if (MoveCount >= 4)
                            {
        ...
        private bool ClientSpellXY(int wIdent, int nKey, short nTargetX, short nTargetY, IActor targetObject, bool boLateDelivery, ref int delayTime)
        {
            ...
            if (!boLateDelivery && !boIsWarrSkill && (!ModuleShare.Config.CloseSpeedHackCheck))
            {
                if (!CheckActionStatus(wIdent, ref delayTime))
                {
                    IsFilterAction = false;
                ...
                if (dwCheckTime < MagicAttackInterval)
                {
                    ...
                    if (delayTime > ModuleShare.Config.MagicHitIntervalTime / 3)
                    {
                        if (MagicAttackCount >= 4)
                        {
                            MagicAttackTick = HUtil32.GetTickCount();
                            MagicAttackCount = 0;
                            delayTime = ModuleShare.Config.MagicHitIntervalTime / 3;
                            if (TestSpeedMode)
                            {
            ...
            if (!boIsWarrSkill)
            {
                MagicAttackInterval = UserMagic.Magic.DelayTime + ModuleShare.Config.MagicHitIntervalTime;
            }
            MagicAttackTick = HUtil32.GetTickCount();
            ushort nSpellPoint;
            bool result;
        ...
        private bool ClientRunXY(int wIdent, short nX, short nY, int nFlag, ref int delayTime)
        {
            ...
            if (nFlag != wIdent && (!ModuleShare.Config.CloseSpeedHackCheck))
            {
                if (!CheckActionStatus(wIdent, ref delayTime))
                {
                    IsFilterAction = false;
                ...
                if (dwCheckTime < ModuleShare.Config.RunIntervalTime)
                {
                    MoveCount++;
                    delayTime = ModuleShare.Config.RunIntervalTime - dwCheckTime;
                    if (delayTime > ModuleShare.Config.RunIntervalTime / 3)
                    {
                        if (MoveCount >= 4)
                        {
                            MoveTick = HUtil32.GetTickCount();
                            MoveCount = 0;
                            delayTime = ModuleShare.Config.RunIntervalTime / 3;
                            if (TestSpeedMode)
                            {
        ...
        private bool ClientWalkXY(int wIdent, short nX, short nY, bool boLateDelivery, ref int delayTime)
        {
            ...
            if (!boLateDelivery && (!ModuleShare.Config.CloseSpeedHackCheck))
            {
                if (!CheckActionStatus(wIdent, ref delayTime))
                {
                    IsFilterAction = false;
                ...
                if (dwCheckTime < ModuleShare.Config.WalkIntervalTime)
                {
                    MoveCount++;
                    delayTime = ModuleShare.Config.WalkIntervalTime - dwCheckTime;
                    if (delayTime > ModuleShare.Config.WalkIntervalTime / 3)
                    {
                        if (MoveCount >= 4)
                        {
                            MoveTick = HUtil32.GetTickCount();
                            MoveCount = 0;
                            delayTime = ModuleShare.Config.WalkIntervalTime / 3;
                            if (TestSpeedMode)
                            {
