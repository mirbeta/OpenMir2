        ...
        private void FlyAxeAttack(BaseObject target) {
            if (Envir.CanFly(CurrX, CurrY, target.CurrX, target.CurrY)) {
                ...
                if (nDamage > 0) {
                    ...
                    target.SendTargetDelayMsg(Messages.RM_STRUCK, Messages.RM_REFMESSAGE, nDamage, target.WAbil.HP, target.WAbil.MaxHP, ActorId, "", HUtil32._MAX(Math.Abs(CurrX - target.CurrX), Math.Abs(CurrY - target.CurrY)) * 50 + 600);
                }
