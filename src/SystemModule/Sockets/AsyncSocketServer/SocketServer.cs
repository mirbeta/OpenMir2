...
namespace SystemModule.Sockets.AsyncSocketServer
{
    ...
    public class SocketServer
    {
        ...
        /// 接收数据事件对象集合
        /// </summary>
        private readonly ConcurrentDictionary<string, AsyncUserToken> _Tokens;
        /// <summary>
        /// 设计同时处理的连接最大数
        /// </summary>
        ...
        public SocketServer(int numConnections, int bufferSize)//构造函数
        {
            ...
            // 接收数据事件参数对象集合
            _Tokens = new ConcurrentDictionary<string, AsyncUserToken>();
        ...
        public void Init(byte writPopRate = 2)
        {
            ...
            for (int i = 0; i < _numConnections * writPopRate; i++)
            {
        ...
        private void StartAccept(SocketAsyncEventArgs acceptEventArg)
        {
            ...
            try
            {
                _maxNumberAcceptedClients.WaitOne(TimeSpan.FromMilliseconds(200));// 对信号量进行一次P操作
        ...
        /// </summary>
        /// <returns></returns>
        private static string BytesToReadableValue(long length)
        {
