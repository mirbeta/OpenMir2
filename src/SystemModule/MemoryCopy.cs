...
namespace SystemModule
{
    public static class MemoryCopy
    {
        ...
        public static unsafe void BlockCopy(nint source, int srcOff, Span<byte> destination, int dstOff, int count, int sourceLen)
        {
            var destinationSpan = new Span<byte>(source.ToPointer(), sourceLen);
        ...
        {
            var destinationSpan = new Span<byte>(source.ToPointer(), count);
            fixed (byte* src = &destinationSpan[srcOff])
            {
                fixed (byte* dest = &destination[dstOff])
                {
