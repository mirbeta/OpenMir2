...
public class MemberSetter
{
    ...
    private static Action<object, object> CreateSetterDelegate(PropertyInfo property)
    {
        ...
        ParameterExpression param_value = Expression.Parameter(typeof(object));

        UnaryExpression body_instance = Expression.Convert(param_instance, property.DeclaringType);
        UnaryExpression body_value = Expression.Convert(param_value, property.PropertyType);
        MethodCallExpression body_call = Expression.Call(body_instance, property.GetSetMethod(true), body_value);
